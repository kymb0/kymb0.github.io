...
<details>
<summary><strong>How does "probability distribution" actually work?</strong></summary>

During training, the model learns weights that determine how strongly each pattern affects the prediction. It sees billions of text examples, learns "when these tokens appear in this order, the next token is usually X," and assigns weights based on how predictive each pattern is. At inference, it multiplies inputs by learned weights and outputs a probability for each possible next token. Highest probability wins.

This is a massive simplification—transformers use attention mechanisms, not simple linear weights—but the core idea holds: learned patterns × input context = prediction.

For a deeper dive into how models learn these weights, see [my post on ML fundamentals](/link-to-supernatural-post/).
...
System prompts are stored server-side and prepended to every request. You never see them in standard client-side API calls you intercep—they're injected by the backend before the combined prompt hits the LLM.
