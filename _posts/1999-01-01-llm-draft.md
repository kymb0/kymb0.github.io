...
If we don't force the explicit Jinja deposit in Turn 4, a literal character-by-character attack often does work on the first run. So the failure isn't inherent to char-by-char delivery; the failure is the dependency conflict between the explicit deposit and the subsequent literal payload. Char-by-char is reliably effective in isolation but it fails in this instance because the attention heads are already on high alert from the T4 deposit. 
